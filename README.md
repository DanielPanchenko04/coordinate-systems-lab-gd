...
**Мета роботи:** Практично реалізувати та дослідити роботу алгоритму градієнтного спуск, аналіз впливу гіперпараметрів, шуму та геометрії на точність позиціонування.

---

## Завдання 1: Контрольний Запуск ("Ідеальний світ")
**Параметри:** `NOISE_LEVEL = 0`, `LEARNING_RATE = 0.01`, `INITIAL_GUESS = [50000, 50000]`, `TRUE_POSITION = [45000, 35000]`.
...
**Мета:** Продемонструвати компроміс (трейдоф) між стабільністю алгоритму та швидкістю його збіжності.

**Налаштування:** `NOISE_LEVEL = 1e-6`, `INITIAL_GUESS = [50000, 50000]`, `TRUE_POSITION = [45000, 35000]`. Послідовна зміна параметра `LEARNING_RATE`.
...
**Мета:** Показати пряму залежність між якістю вхідних даних (рівнем шуму) та точністю результату позиціонування .

**Налаштування:** `LEARNING_RATE = 0.01`, `INITIAL_GUESS = [50000, 50000]`, `TRUE_POSITION = [45000, 35000]`. Послідовна зміна параметра `NOISE_LEVEL`.
...
1. **Поясніть, чому в експерименті 3c алгоритм все одно сходиться (графік Loss падає), але фінальна похибка позиціонування є катастрофічною? До якого мінімуму він збігся?**
...
**Налаштування:** `NOISE_LEVEL = 1e-6`, `LEARNING_RATE = 0.01`, `TRUE_POSITION = [45000, 35000]`. Послідовна зміна параметра `INITIAL_GUESS`.
...
**Мета:** Продемонструвати, як погана геометрія базових станцій (GDOP) робить задачу чисельно нестійкою і катастрофічно посилює вплив існуючого шуму на кінцевий результат.

**Налаштування:** `NOISE_LEVEL = 1e-6`, `LEARNING_RATE = 0.01`, `INITIAL_GUESS = [50000, 50000]`. Зміна цільової позиції об'єкта `TRUE_POSITION`.
...
**Налаштування:** Погана геометрія `TRUE_POSITION = [45000, -135000]`, `NOISE_LEVEL = 1e-6`, `LEARNING_RATE = 0.01`. Зміна параметра `TOLERANCE` та `MAX_ITERATIONS`.
